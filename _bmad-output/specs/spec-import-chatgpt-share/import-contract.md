...
La V1 enregistre uniquement `chatgpt`. Son adaptateur possède la validation stricte, les chemins d’extraction et les classifications décrits ci-dessous. La récupération distante consentie est séparée de cet adaptateur dans `remoteGateway` : elle lui remet seulement un HTML borné à extraire. Claude, Gemini et Mistral ne sont ni affichés comme disponibles ni acceptés par validation en V1. Leur ajout ultérieur consiste à fournir leur adaptateur — avec ses limites publiques, CORS et formats spécifiques — puis ses jeux de tests ; il ne modifie pas le contrat d’événements normalisés ni le calcul environnemental.

## Parcours et états

L’interface ajoute une zone d’import avant les blocs : fournisseur sélectionné (ChatGPT seul en V1), champ URL, action « Analyser le lien », consentement distant, prévisualisation, avertissements et action « Remplacer les échanges par l’import ». Après validation de l’URL et avant toute requête via tiers, un dialogue accessible présente `corsproxy.io`, sa politique et ses conditions, la finalité de récupération de la page publique, l’URL transmise, ainsi que les métadonnées de requête qu’il peut recevoir (dont adresse IP et agent utilisateur). Il dit explicitement que les blocs déjà saisis, fichiers locaux, résultats et paramètres de calcul ne sont pas envoyés par le calculateur. Les actions sont « Continuer avec corsproxy.io », « Annuler » et « Importer manuellement » ; le focus entre dans le dialogue, `Escape` et Annuler le ferment sans requête. Le consentement est non pré-coché, ponctuel et invalidé par toute modification d’URL. Une confirmation distincte est obligatoire si un bloc non vide existe. L’échec conserve intégralement les blocs présents. Après validation, les blocs importés sont des blocs ordinaires, éditables, supprimables et recalculables ; aucun calcul n’est lancé automatiquement.

Le navigateur valide exactement le schéma `https`, l’hôte `chatgpt.com`, le chemin `/share/<id>` alphanumérique avec tirets, sans port, identifiants, query ni fragment. Après consentement seulement, `application/import/remoteGateway` construit la requête depuis cette unique valeur vers l’origine allowlistée `https://corsproxy.io/`, avec le mécanisme de clé API et d’autorisation de domaine documenté par le fournisseur. Aucun hôte, chemin proxy, URL de destination, cookie applicatif, état de reducer, fichier local ou résultat ne provient d’une entrée utilisateur ni ne rejoint cette requête. La passerelle borne délai et taille, retourne des erreurs discriminées de consentement, politique/configuration, réseau, HTTP, délai et taille, puis transmet le HTML comme texte à l’extracteur existant. Elle ne suit pas de lien, ne charge aucune ressource liée et ne manipule jamais le contenu distant avec `innerHTML`. Une réponse de format inconnu est refusée localement et atomiquement.
...
- Tester que `remoteGateway` accepte seulement l’URL ChatGPT validée, construit exclusivement une requête vers `https://corsproxy.io/`, applique délai et taille, retourne des erreurs typées et ne reçoit aucun bloc, fichier, résultat, paramètre, cookie applicatif ou jeton de session.
