...
> **Contrat canonique.** Cette SPEC et ses compagnons constituent le contrat complet, validé pour la préservation, à construire et tester. Elle complète le calculateur existant ; elle lève le non-objectif d’import uniquement après l’étude de faisabilité fournie.

# Importer une conversation partagée

## Why

La saisie manuelle d’une conversation est longue et oublie facilement les traces visibles de raisonnement, d’outils et d’artifacts qui influencent le calcul. Une personne qui possède un lien public ChatGPT doit pouvoir préremplir fidèlement les échanges calculables, tout en gardant le calculateur statique, privé et honnête sur ce que le partage ne rend pas accessible.
...
- **CAP-1 — Importer un partage public**
  ...
  - **success:** Seule `https://chatgpt.com/share/<id>` est acceptée ; une lecture réussie crée une prévisualisation importable, et tout refus réseau, CORS, format inconnu ou dépassement de limite est expliqué sans importer de données partielles comme certaines.
...
- Publication statique GitHub Pages, sans compte, serveur, proxy, cookie, jeton, authentification, API ou contournement CORS ; l’import direct doit échouer proprement si ChatGPT ne permet pas la lecture cross-origin.
- L’URL, le HTML public, les messages extraits et les fichiers restent seulement en mémoire de la session : aucun stockage durable, analytics, journal distant, contenu en URL ou envoi à un tiers.
...
- Contourner des protections d’accès, CORS, authentification ou droits de téléchargement de ChatGPT, ni récupérer le raisonnement invisible ou le contenu non public.
...
- Une personne peut importer, compléter par upload/collage ce qui manque, puis lancer les mêmes calculs locaux qu’après une saisie manuelle, sans requête de contenu vers un autre service que la page de partage choisie.

## Assumptions

- La page publique autorise une lecture `fetch` depuis le navigateur servi sur GitHub Pages. Dans le cas contraire, l’import par URL reste indisponible dans cette architecture et le message d’échec, sans contournement, est le comportement attendu.
