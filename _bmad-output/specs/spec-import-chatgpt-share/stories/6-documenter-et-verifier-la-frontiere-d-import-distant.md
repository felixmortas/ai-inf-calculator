...
**Approche :** Mettre à jour l’aide utilisateur avec les faits vérifiés sur `corsproxy.io` et les quatre fournisseurs, puis compléter la couverture d’intégration de la frontière : résolution en `ResolvedShare`, consentement lié à son identité, requête minimale, règles de redirection, parsing local, échec atomique et continuité du parcours manuel.

## Boundaries & Constraints

**Always :** Citer la documentation, la politique de confidentialité et les conditions de `corsproxy.io`. Pour chaque adaptateur, documenter son URL publique admise, ses limites, sa politique de redirection et les contenus non accessibles. Déclarer que le tiers reçoit au minimum l’URL canonique et peut recevoir IP, agent utilisateur, horodatages et compteurs de requêtes ; ses conditions permettent le traitement du contenu proxyé nécessaire au service. Documenter l’absence de transmission par le calculateur des blocs, fichiers, résultats et paramètres locaux, sans conclure que le tiers ne traite ni ne conserve jamais la page. Rappeler le consentement à usage unique lié au `ResolvedShare`, à `policyVersion` et à l’origine proxy, ainsi que la revue D-4 avant publication.
...
- [ ] Mettre à jour l’aide d’import multi-fournisseur avec le consentement, les données transmises, les règles par adaptateur, les liens tiers et le parcours manuel.
- [ ] Ajouter les tests d’intégration du parcours résolution → consentement → passerelle → parsing local et de ses erreurs.
...
- Given une demande distante consentie, when son contenu est vérifié, then seul l’`outboundCanonicalUrl` du `ResolvedShare` validé quitte le calculateur ; fichiers, blocs, résultats et paramètres locaux restent absents de la requête.
...
- Given les fixtures de ChatGPT, Claude, Mistral et Gemini, when leurs URL, redirections, limites, états absents, structures inconnues et contenus non textuels sont exercés, then chaque échec est typé, atomique et isolé au fournisseur concerné.
