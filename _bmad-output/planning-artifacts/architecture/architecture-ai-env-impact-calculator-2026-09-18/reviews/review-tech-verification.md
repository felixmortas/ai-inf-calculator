...
| Aucune | Vite accepte `base: '/ai-inf-calculator/'`, ce qui rebased les actifs du build pour une publication au sous-chemin prévu. | Conserver cette base ; la vérifier sur l’URL finale `https://felixmortas.com/ai-inf-calculator/`. |
| À surveiller | La publication directe depuis une branche GitHub Pages ne construit pas Vite. Elle ne peut fonctionner que si le build est commité dans le dossier/branche publié. Pour agréger le site racine et `ai-inf-calculator/dist`, GitHub Actions est la voie native documentée. | À l’intégration, adopter un workflow Pages unique qui construit le site hôte et `ai-inf-calculator/`, puis déploie l’artefact combiné. Ce point est correctement différé dans la spine. |
