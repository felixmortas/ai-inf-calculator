...
import { MISTRAL_SHARE_LIMITS } from './mistralShare';
...
describe('passerelle distante bornée', () => {
  ...
  it('ignore les prédicats permissifs fournis par l’appelant', () => {
    const forged = Object.freeze({ ...resolved });
    expect(createRemoteGatewayConsent(forged, (_value): _value is typeof resolved => true, () => ({ id: 'mistral', label: 'Mistral', limits: resolved.limits, policyVersion: resolved.policyVersion, redirectPolicy: { maxRedirects: 0, allowedOrigins: ['https://chat.mistral.ai'] }, validateUrl: () => undefined, importFromUrl: async () => ({ ok: true as const, providerId: 'mistral', events: [] }) }))).toBeUndefined();
  });
  ...
  it('ne part pas sans configuration et distingue réseau, HTTP, délai et taille', async () => {
    ...
    await expect(createRemoteGateway(vi.fn().mockResolvedValue(response('x', 200, { 'content-length': String(MISTRAL_SHARE_LIMITS.maxBytes + 1) })), configured).fetchHtml(resolved, consent())).resolves.toMatchObject({ ok: false, error: { code: 'response-too-large' } });
  });
  ...
  it('annule atomiquement le corps refusé ou trop grand', async () => {
    ...
    const oversized = new Response(new ReadableStream<Uint8Array>({ cancel: canceled }), { headers: { 'content-length': String(MISTRAL_SHARE_LIMITS.maxBytes + 1) } });
  ...
  it('retourne un délai atomique si fetch ou la lecture du corps reste bloqué', async () => {
    ...
    try {
      ...
      await vi.advanceTimersByTimeAsync(MISTRAL_SHARE_LIMITS.timeoutMs);
  ...
  it('annule un flux sans content-length dès que ses chunks franchissent la borne', async () => {
    ...
    const body = new Response(new ReadableStream<Uint8Array>({
      start(controller) { controller.enqueue(new Uint8Array(MISTRAL_SHARE_LIMITS.maxBytes)); },
  ...
  it('refuse Gemini avant tout consentement et fetch', async () => {
  ...
  it.each([shareUrl])('poste uniquement le lien attesté du fournisseur %s', async (url) => {
